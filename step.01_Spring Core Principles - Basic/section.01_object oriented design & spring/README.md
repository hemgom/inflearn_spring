...
- 다시 스프링으로
  - 스프링 이야기에 왜 객체 지향 이야기가 나오는가?
    - 스프링은 아래의 기술로 `다형성 + OCP, DIP`를 가능하게 지원
      - `DI(dependency injection)` : 의존관계, 의존성 주입
      -  DI 컨테이너 제공
    - `클라이언트 코드 변경` 없이 기능 확장 가능
  - 스프링이 없던 시절
    - 좋은 객체 지향 개발을 위해 `OCP, DIP`를 지키며 개발하니 배 보다 배꼽이 큰 상황이 계속 생겨 프레임워크를 만듬
    - `OCP, DIP`를 지키며 개발하다 보면 결국 `스프링 프레임워크(정확히 DI 컨테이너)`를 만들게 됨

- 정리
  - 모든 설계에 `역할`과 `구현`을 분리하자!
  - 언제든 `유연`하게 `변경`할 수 있도록 설계하는 것이 좋은 객체 지향 설계
  - 이상적으로는 모든 설계에 인터페이스를 부여하는 걸 목표로 하자!
  - 실무에 대한 고민
    - 인터페이스 도입 시 `추상화 비용`이 발생함
    - 기능 확장 가능성이 없다면 구체 클래스를 직접 사용하고, 향후에 꼭 필요할 때 리팩러링해 인터페이스를 도입하는 방법도 있음