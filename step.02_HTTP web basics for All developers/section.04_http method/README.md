...
- `안전`의 경우 해당 `리소스`만 고려하며 그 외의 부분을 고려하진 않음  
<br/>

### 멱등 (Idempotent)
`f(f(x)) = f(x)` : 즉, 몇 번을 호출해도 그 결과가 똑같다.
- 해당 메서드
	- GET : 몇 번을 조회해도 같은 결과가 조회 됨
	- PUT : 결과를 대체함, 같은 요청을 몇 번을 하건 최종적으로 결과가 같음
	- DELETE : 결과를 삭제함, 같은 요청을 몇 번을 해도 삭제된 결과는 같음
	- `주의!` : POST의 경우 멱등 메서드처럼 보일 수 있으나 아님
			- ex) 결제를 여러번 하면 그 결과는 전혀 같지 않음
- 활용
	- 자동 복구 메커니즘
		- 서버가 정상 응답을 못 했을 때, `클라이언트가 같은 요청을 다시 보내도 되는가`에 대한 판단 근거
		- `멱등`은 같은 요청을 몇 번하더라도 그 결과가 같으므로
- `주의!` : 멱등은 `같은 요청을 여러번 했을 때`의 경우만 고려함
	- 즉, 외부 요인으로 인해 중간에 리소스가 변경되는 등의 고려 따위는 하지 않음
<br/>

### 캐시가능 (Cacheable)
응답 결과 리소스를 `캐시`해서 사용해도 되는가?
- 해당 메서드 : GET, HEAD, POST, PATCH 캐시가능
	- 하지만 실제로 GET, HEAD 정도만 캐시로 사용
	- POST, PATCH는 `message body(본문내용)`까지 키로 고려해야 하므로 구현이 어려움